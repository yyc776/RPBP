...
class ByproductModel(nn.Module):
    def __init__(self, args, device):
        super(ByproductModel, self).__init__()
        self.args = args
        self.device = device
...
class Trainer:
    ...
    def train_epochsample(self, train_dataloader, epoch):
        ...
        for i, data in enumerate(progress_bar):
            ...
            correct += cur_batch_size * acc.detach()
        ...
        labels = torch.cat(labels_list, dim=0)
        acc, macro_F, gmean, bacc = performance_measure(output_list, labels)
