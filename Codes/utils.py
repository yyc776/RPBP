...
def get_label_dict(path=r'USPTO-50K/dataset/stage_one/by-product_smiles_to_label.dict'):
    label_dict = joblib.load(path)
    return label_dict
...
def check_atommap(smiles):
    atom_map_numbers = list(map(int, re.findall(r"(?<=:)\d+", smiles)))
    smiles_atoms = Chem.MolFromSmiles(smiles).GetNumAtoms()
    return atom_map_numbers.max() == smiles_atoms
